...
const uni = [];
for (let i = 0; i < graduates.length; i++) {
  uni.push(graduates[i].university.search("uni")); 
}
console.log(uni);
...
*/
const animalNames = [];
zooAnimals.forEach(function(param) {
  animalNames.push(`Name: ${param.animal_name}, Scientific: ${param.scientific_name}`)
});
...
let lowerCase = [];
lowerCase = zooAnimals.map(function(param){
  return `${param.animal_name.toLowerCase()}`;
});
...
largerPopulation = zooAnimals.filter(function(param){
